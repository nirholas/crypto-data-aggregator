...
| SDK | Source | Installation | Features |
| --- | --- | --- | --- |
| **TypeScript** | [`sdk/typescript/`](sdk/typescript/) | `npm install @nirholas/crypto-news` | Full types, SWR integration |
| **React** | [`sdk/react/`](sdk/react/) | Not published yet. Build from source. | Hooks: `useCryptoNews`, `usePrice` |
| **JavaScript** | [`sdk/javascript/`](sdk/javascript/) | Not published yet. Copy the module. | Browser and Node.js compatible |
| **Python** | [`sdk/python/`](sdk/python/) | Not published yet. Copy `crypto_news.py`. | Zero dependencies, async support |
| **Go** | [`sdk/go/`](sdk/go/) | Not published yet. Vendor the package. | Goroutine-safe, context support |
| **PHP** | [`sdk/php/`](sdk/php/) | Not published yet. Copy `CryptoNews.php`. | PSR-4, Guzzle HTTP |

Only the TypeScript SDK is on npm today. The rest ship as source in this
repository; each folder has its own README with a runnable example.
...
The CLI lives in [`cli/`](cli/) and is not published to npm yet, so run it from
a clone:
...
**Crypto Data Aggregator** is deployed and accessible over HTTP via [MCP Streamable HTTP](https://modelcontextprotocol.io/specification/2025-03-26/basic/transports#streamable-http) transport — no local installation required.

**Endpoint:**
```
https://modelcontextprotocol.name/mcp/crypto-data-aggregator
```
...
**Get crypto prices:**
```bash
curl -X POST https://modelcontextprotocol.name/mcp/crypto-data-aggregator \
  -H "Content-Type: application/json" \
  -d '{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"get_price","arguments":{"ids":"bitcoin,ethereum,solana","vs_currencies":"usd"}}}'
```
...
curl -X POST https://modelcontextprotocol.name/mcp/crypto-data-aggregator \
  -H "Content-Type: application/json" \
  -d '{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"get_market_overview","arguments":{"limit":20}}}'
```
...
- **This repository**: [github.com/nirholas/crypto-data-aggregator](https://github.com/nirholas/crypto-data-aggregator). The hosted demo and the SperaxOS marketplace listing are offline while hosting is being migrated.
